...
function render(){const root=document.getElementById('root');if(page==='home'){root.innerHTML=`<div class="min-h-screen bg-green-50 text-gray-800 p-4 sm:p-6 lg:p-8" style="background-image:url('data:image/svg+xml,%3Csvg width=\'20\' height=\'20\' viewBox=\'0 0 20 20\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'%2322c55e\' fill-opacity=\'0.1\' fill-rule=\'evenodd\'%3E%3Ccircle cx=\'3\' cy=\'3\' r=\'3\'/%3E%3Ccircle cx=\'13\' cy=\'13\' r=\'3\'/%3E%3C/g%3E%3C/svg%3E')"><div class="max-w-4xl mx-auto"><header class="text-center my-8 sm:my-12"><h1 class="text-4xl sm:text-5xl md:text-6xl font-bold text-emerald-800">Pomar de Casa</h1><p class="mt-4 text-lg text-emerald-700">Seu guia para um pomar saudável e produtivo.</p></header><form class="relative mb-12"><input id="searchInput" type="text" placeholder="Procure por uma fruta ou árvore..." class="w-full pl-12 pr-4 py-4 text-lg bg-white border-2 border-green-200 rounded-full shadow-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition duration-300"/><div class="absolute left-4 top-1/2 -translate-y-1/2">${svgSearch()}</div></form><section><h2 class="text-2xl font-bold text-emerald-800 mb-6 text-center">Ou explore algumas populares:</h2><div class="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">${[{name:'Laranjeira',image:'1084'},{name:'Limoeiro',image:'102'},{name:'Macieira',image:'1079'},{name:'Mangueira',image:'219'},{name:'Abacateiro',image:'429'},{name:'Jabuticabeira',image:'431'}].map(t=>`<div data-name="${t.name}" class="group relative rounded-xl overflow-hidden shadow-lg cursor-pointer transform hover:scale-105 transition-transform duration-300"><img src="https://picsum.photos/id/${t.image}/400/400" alt="${t.name}" class="w-full h-full object-cover"/><div class="absolute inset-0 bg-black bg-opacity-40 group-hover:bg-opacity-20 transition-colors duration-300"></div><h3 class="absolute bottom-4 left-4 text-white text-xl font-bold">${t.name}</h3></div>`).join('')}</div></section></div><button id="chatBtn" class="fixed bottom-6 right-6 bg-emerald-600 text-white p-4 rounded-full shadow-lg hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 transform hover:scale-110 transition-all duration-300 z-50 flex items-center gap-2" aria-label="Converse com o Jardineiro Sábio">${svgChat()}<span class="hidden sm:inline">Jardineiro Sábio</span></button></div>`;const input=document.getElementById('searchInput');input.addEventListener('input',()=>{const q=normalize(input.value);if(!q){return}const found=frutas.find(f=>normalize(f.nome).includes(q)||normalize(f.cientifico).includes(q));if(found){selected=found;page='detail';renderDetail()}});document.querySelectorAll('[data-name]').forEach(el=>el.addEventListener('click',()=>{selected=frutas.find(f=>normalize(f.nome).includes(normalize(el.getAttribute('data-name'))))||{nome:el.getAttribute('data-name'),cientifico:'',frutificacao:'',nutriente:'',adubacao:''};page='detail';renderDetail()}));document.getElementById('chatBtn').addEventListener('click',()=>{page='chat';renderChat()})}}

async function getTreeInfo(name){const key=localStorage.getItem('GEMINI_API_KEY')||window.GEMINI_API_KEY||'';if(!key){return null}const mod=await import('@google/genai');const ai=new mod.GoogleGenAI({apiKey:key});const schema={type:mod.Type.OBJECT,properties:{name:{type:mod.Type.STRING},scientificName:{type:mod.Type.STRING},description:{type:mod.Type.STRING},care:{type:mod.Type.OBJECT,properties:{sunlight:{type:mod.Type.STRING},watering:{type:mod.Type.STRING},soil:{type:mod.Type.STRING}},required:['sunlight','watering','soil']},pruningHarvest:{type:mod.Type.STRING},pests:{type:mod.Type.ARRAY,items:{type:mod.Type.OBJECT,properties:{name:{type:mod.Type.STRING},description:{type:mod.Type.STRING}},required:['name','description']}}},required:['name','scientificName','description','care','pruningHarvest','pests']};const resp=await ai.models.generateContent({model:'gemini-2.5-flash',contents:`Forneça informações detalhadas sobre a árvore frutífera: ${name}. Responda em Português do Brasil.`,config:{responseMimeType:'application/json',responseSchema:schema}});const jsonText=resp.text.trim();return JSON.parse(jsonText)}
...
async function init(){await loadCSV();loadUserFrutas();mergeUserFrutas();loadImageCache();render();removeInitialPicsum();updateFeaturedIcons();ensureSearchWatcher();ensureIconsWatcher();ensureAddButtonWatcher();ensureFeaturedImagesOnce()}
