...
function requirePublisherAuth(){
  try{
    const session=(localStorage.getItem('publisher_session')||'').toUpperCase()
    if(session&&PUBLISHERS[session]){return true}
  }catch{}
  const login=(prompt('Login do publicador:')||'').trim().toUpperCase()
  if(!login||!PUBLISHERS[login]){alert('Publicador não autorizado.');return false}
  const senha=prompt('Senha:')||''
  if(senha!==PUBLISHERS[login]){alert('Senha incorreta.');return false}
  try{localStorage.setItem('publisher_session',login)}catch{}
  alert(`Bem-vindo, ${login}.`)
  return true
}
function handleAddClick(){
  if(!requirePublisherAuth()) return
  openAddModal()
}
